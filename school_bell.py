...
class SchoolBell(QMainWindow):
    ...
    def import_all_settings(self):
        """Импортирует все настройки приложения из YAML-файла."""
        ...
        try:
            ...
            preferences = imported.get("preferences", {})

            if isinstance(schedule, dict) and schedule:
                self.config.schedule_data = schedule
                self.config.save_schedule(schedule)
            ...
            QMessageBox.information(self, "OK", self.tr("status_ready", "Ready"))
        except Exception as e:
            QMessageBox.critical(self, self.tr("error_title", "Error"), str(e))
    ...
    def play_break_music(self, event_time=None):
        ...
        music_settings = self.config.get_music_settings()
        self.music_player.set_selected_tracks(music_settings.get("selected_tracks", []))
        ...
        selected_tracks = music_settings.get("selected_tracks", [])
        self.music_player.set_selected_tracks(selected_tracks)
    ...
    def show_music_settings(self):
        ...
        if dlg.exec() == QDialog.Accepted:
            ...
            if music.get("folder"):
                self.music_player.set_music_folder(music["folder"])
            self.music_player.set_selected_tracks(music.get("selected_tracks", []))
            self.config.save_preferences(self.config.preferences)
    ...
    def closeEvent(self, event):
        ...
        if self.tray_icon and QSystemTrayIcon.isSystemTrayAvailable():
            ...
            cancel_button = dialog.addButton(self.tr("btn_cancel", "Cancel"), QMessageBox.RejectRole)
